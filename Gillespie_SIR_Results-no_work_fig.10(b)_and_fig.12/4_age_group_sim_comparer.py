...
def plot_age_group_comparison_two_simulations():
    ...
    - Gillespie_4AgeGroups_{country}.csv        → solid lines + error bars (No_work scenario)
    - Gillespie_4AgeGroups_{country}_all.csv    → scatter points + error bars (All contacts scenario)
    """
    ...
    for age_idx, (age_group, age_label) in enumerate(zip(age_groups, age_group_labels)):
        ...
        for country_idx, country in enumerate(countries):
            ...
            # ===== Solid line: No_work scenario (mean + error bars) =====
            ax.plot(
                beta_line,
                df_line[mean_col],
                ...
                linestyle='-',
                alpha=0.8,
                label=f'{country} (Sim-No_work)' if age_idx == 0 else ''
            )
...
def plot_single_country_four_groups(country_name):
    ...
    for idx, (age_group, age_label) in enumerate(zip(age_groups, age_group_labels)):
        ...
        # ===== Solid line: No_work (mean + error bars) =====
        ax.plot(
            df_line['Beta'],
            df_line[mean_col],
            color=color_line,
