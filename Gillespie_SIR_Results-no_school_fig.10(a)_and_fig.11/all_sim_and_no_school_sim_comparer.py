...
def plot_simulation_comparison():
    ...
    for i, country in enumerate(countries):
        ...
        # 读取没有学校场景数据 (No school)
        no_school_file = f"Gillespie_SIR_{country}.csv"
        if os.path.exists(no_school_file):
            df_no_school = pd.read_csv(no_school_file)
            beta_no_school = df_no_school['Beta']
            mean_no_school = df_no_school['mean_infected_ratio']
            ci95_low_no_school = df_no_school['ci95_low']
            ci95_high_no_school = df_no_school['ci95_high']

            # 绘制实线 (No school)
            plt.plot(beta_no_school, mean_no_school,
                     color=colors[i], linewidth=2,
                     label=f'{country} (Sim-No_school)', linestyle='-')

            # 添加误差条 (No school)
            y_low = mean_no_school - ci95_low_no_school
            y_high = ci95_high_no_school - mean_no_school
            y_low = np.maximum(y_low, 0)
            y_high = np.maximum(y_high, 0)

            plt.errorbar(beta_no_school, mean_no_school,
                         yerr=[y_low, y_high],
                         fmt='none',
                         ecolor=colors[i],
                         alpha=0.3,
    ...
    plt.savefig('simulation_comparison_all_vs_no_school.png', dpi=300, bbox_inches='tight')
    plt.show()
